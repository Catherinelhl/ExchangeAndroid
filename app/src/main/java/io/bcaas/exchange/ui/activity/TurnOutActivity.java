...
public class TurnOutActivity extends BaseActivity
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (memberKeyVO != null) {
                    ...
                    if (currencyListVO != null) {
                        ...
                        //初始化数据
                        TurnOutView withDrawView = new TurnOutView(this);
                ...
                case Constants.RequestCode.REQUEST_CODE_CAMERA_SCAN:
                    ...
                    if (bundle != null) {
                        ...
                        if (ListTool.noEmpty(views) && currentPosition < views.size()) {
                            if (ListTool.noEmpty(memberKeyVOList)) {
                                ((TurnOutView) views.get(currentPosition)).setScanInfo(scanInfo);
                            }
                        }
                    }
                    break;
        ...
        if (tabLayout != null) {
            if (ListTool.noEmpty(views)) {
                for (int i = 0; i < views.size(); i++) {
                    if (ListTool.noEmpty(memberKeyVOList)) {
                        ((TurnOutView) views.get(i)).refreshData(memberKeyVOList.get(i));
    ...
    private void refreshView() {
        if (ListTool.noEmpty(views)
            ...
            ((TurnOutView) views.get(currentPosition)).refreshData(memberKeyVOList.get(currentPosition));
        }
    }
}
