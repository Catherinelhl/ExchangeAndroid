...
import static io.bcaas.exchange.tools.time.DateFormatTool.getUTCDateForAMPMFormat2;
...
//                            startTime = getPastTimeOfStartByCycleTime(cycleTimeType).getTime();
        ...
        //默认获取所有的数据信息
        presenter.getCoinMarketCap(coinName, 0l, 0l);
...
//            float val = (float) (Math.random() * range) - 30;
    ...
    public void onValueSelected(Entry e, Highlight h) {
        ...
        if (priceUSD != null) {
            tvTimer.setText("Time: " + getUTCDateForAMPMFormat2(priceUSD.get(index).get(0)));
...
//        count = valueMarket.size();
        LogTool.d(TAG, "lineBean:" + coinMarketCapBean.getMarket_cap_by_available_supply());
//        priceBTC = coinMarketCapBean.getPrice_btc();
        LogTool.d(TAG, "priceBTC:" + coinMarketCapBean.getPrice_btc());
//        priceUSD = coinMarketCapBean.getPrice_usd();
        LogTool.d(TAG, "priceUSD:" + coinMarketCapBean.getPrice_usd());
//        volumeUSD = coinMarketCapBean.getVolume_usd();
        LogTool.d(TAG, "volumeUSD:" + coinMarketCapBean.getVolume_usd());
        initLineChart();
    }
